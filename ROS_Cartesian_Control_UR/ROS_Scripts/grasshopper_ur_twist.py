...
def listener():
    rospy.Subscriber("/target_plane_twist", Twist, callback)
    rospy.spin()


def callback(data):

    target_twist = [data.linear.x, data.linear.y, data.linear.z, data.angular.x, data.angular.y, data.angular.z]
    print (target_twist)
    
    # Random 2-point trajectory
    duration = 10
    x, y, z, ax, ay, az = target_twist
