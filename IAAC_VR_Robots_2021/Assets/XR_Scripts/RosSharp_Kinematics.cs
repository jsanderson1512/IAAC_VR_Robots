...
public class RosSharp_Kinematics : MonoBehaviour
{
    ...
    //rlj debug hack, undo
    private double[] lastAngles = {-69f, -94f, -103f, -252f, -89f, 20f }; //store the last known angles so that we can choose the correct kinematic configuration, or at the very least stay put if we can't reach a point
    ...
    void Start()
    {
        ...
        else if (robotModel == RobotModel.UR_10e)//UR10e
        {
            a = aUR10e;
            d = dUR10e;
        }
    ...
    void Update()
    {
        ...
        if (movementMode == MovementMode.RandomKinematics)
        {
            for (int i = 0; i < 6; i++)
            {
        ...
        else if(robotModel == RobotModel.UR_10e)
        {
            ...
            {
                float jointAngleRad = jointAngles[i] * Mathf.Deg2Rad;
               // Debug.Log("IK joint angle " + i + " = " + jointAngleRad);
                JointStateWriters[i].Write(jointAngleRad);
            }
        }
    } //end Update
